...
def parse_query(line):
    ...
    if len(clauses) > 1:
        execute(columns, functions, distincts, dist_pair, needed, tables_inquery, clauses[1])
    else:
        execute(columns, functions, distincts, dist_pair, needed, tables_inquery)

def execute(columns, functions, distincts, dist_pair, needed, tables_inquery, clauses=[]):
    '''
        Execute depending on parameters passed into this function
    '''
    if len(tables_inquery) == 1:
